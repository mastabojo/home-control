...
for count in range(0, maxTries):
    addr = ADDR_INPUT_FREQUENCY_30071
    try:
        ## print desc + ': ' + str(instrument.read_float(addr, READ_INPUT_REGISTER))
        frequency = instrument.read_float(addr, READ_INPUT_REGISTER)
...
phase_1_voltage = 0.0;
for count in range(0, maxTries):
    addr = ADDR_INPUT_PHASE1_TO_NEUTRAL_30001
    try:
        ## print desc + ': ' + str(instrument.read_float(addr, READ_INPUT_REGISTER))
        phase_1_voltage = instrument.read_float(addr, READ_INPUT_REGISTER)
        time.sleep(sleepTime)
        if phase_1_voltage > 0.0:
            break;
    except IOError:
        print("Failed to read address " + str(addr))

print str(phase_1_voltage)
...
for count in range(0, maxTries):
    ...
    try:
        ...
        phase_1_voltage = instrument.read_float(addr, READ_INPUT_REGISTER)
        time.sleep(sleepTime)
        if phase_2_voltage > 0.0:
            break;
    except IOError:
        print("Failed to read address " + str(addr))

print str(phase_2_voltage)

phase_3_voltage = 0.0;
for count in range(0, maxTries):
    addr = ADDR_INPUT_PHASE3_TO_NEUTRAL_30005
    try:
        ## print desc + ': ' + str(instrument.read_float(addr, READ_INPUT_REGISTER))
        phase_1_voltage = instrument.read_float(addr, READ_INPUT_REGISTER)
        time.sleep(sleepTime)
        if phase_3_voltage > 0.0:
            break;
    except IOError:
        print("Failed to read address " + str(addr))

print str(phase_3_voltage)
